#!/usr/bin/env python3
"""Generate standardized 27D features from processed bounding-box tracks.

Bounding boxes are Kalman-filtered before feature extraction. Scaling is fitted
on the training split and applied to both splits.

Usage:
    python src/scripts/preprocess_data.py --dataset shanghaitech --split all
"""
import os
import sys
import argparse
...
from utils.bbox_features import BBoxFeatureExtractor
...
def load_video_tracks(parquet_path: Path) -> Dict[int, pd.DataFrame]:
    """
    Load tracks from parquet file and group by track_id.

    Returns:
        Dict[track_id -> DataFrame with columns: frame_id, x1, y1, x2, y2, conf, class_id]
    """
...
def apply_kalman_to_tracks(
    ...
    """Apply Kalman filtering to all tracks."""
    smoothed_tracks = {}
...
def process_video(
    ...
    """
    video_name = parquet_path.stem
    tracks = load_video_tracks(parquet_path)
    ...
    if len(all_x_cont) == 0:
        return (np.empty((0, SEG_LEN, 16), dtype=np.float32),
    ...
    return x_cont, x_cat, all_metadata
...
def process_split(
    ...
    Args:
        dataset_name: Dataset name (e.g., 'shanghaitech', 'ubnormal')
        vid_res: Video resolution (width, height)
        split: 'training' or 'testing'
        apply_kalman: Whether to apply Kalman filtering
        feature_config: Config dictionary (determines variant name automatically)
    ...
    if feature_config is not None:
        ...
        apply_log = feature_config.get('extractor', {}).get('apply_log', True)
    ...
    input_dir = DATA_ROOT / f"{dataset_name}_tracks" / f"{dataset_name}_tracks_processed" / split
    output_dir = DATA_ROOT / f"{dataset_name}_tracks" / f"{dataset_name}_tracks_features" / f"variant_{feature_variant}" / split
    variant_dir = DATA_ROOT / f"{dataset_name}_tracks" / f"{dataset_name}_tracks_features" / f"variant_{feature_variant}"
    ...
    # Note: This is a safety net. In practice, frame extraction already filters out abnormal videos.
    if split == 'training':
    ...
    for pq_file in tqdm(parquet_files, desc=f"[{split}]"):
        x_cont, x_cat, metadata = process_video(
            pq_file, extractor, apply_kalman, seg_stride
        )
...
def main():
    parser = argparse.ArgumentParser(description="Feature extraction for trajectory VAD")
    ...
    for apply_kf, version in versions:
        ...
        # Always process training first (to fit scaler)
    ...
    for r in results:
        if r['num_segments'] > 0:
            print(f"{r['split']:<12} {r['version']:<8} {r['num_segments']:<12} "
                  f"{str(r['shape']):<20} {r['file_size_mb']:<10.2f}")

    print("=" * 70)
    print("\n✓ Feature extraction complete!")
    output_base = DATA_ROOT / f"{dataset_config.name}_tracks" / f"{dataset_config.name}_tracks_features" / f"variant_{feature_variant}"
